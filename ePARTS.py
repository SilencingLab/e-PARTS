...
class Application1(Frame): #Welcome Screen where identification in checked
	def __init__(self,master):
		...
		self.lbl_ip=Label(text="Host IP:").grid(row=3,sticky=W)
		self.lbl_schema=Label(text="Database schema:").grid(row=4,sticky=W)
		...
		self.ent_ip=Entry(master) #Entry where writting the user name
		self.ent_ip.grid(row=3,column=1)
		
		self.ent_schema=Entry(master) #Entry where writting the user name
		self.ent_schema.grid(row=4,column=1)
	...
	def check_identity(self):
		...
		self.ip=self.ent_ip.get() #get IP
		self.schema=self.ent_schema.get() #get schema
		
		config = {
			'user':user_name,
			'passwd':self.pwd,
			'host':self.ip, #Server IP
...
WelcomeScreen.geometry("420x180")
